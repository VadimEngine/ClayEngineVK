...
uint32_t GraphicsContextDesktop::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void GraphicsContextDesktop::createDescriptorPool() {
    ...
    poolSizes[0].descriptorCount = static_cast<uint32_t>(GraphicsContextDesktop::MAX_FRAMES_IN_FLIGHT);
    poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[1].descriptorCount = static_cast<uint32_t>(GraphicsContextDesktop::MAX_FRAMES_IN_FLIGHT);

    VkDescriptorPoolCreateInfo poolInfo{};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.poolSizeCount = static_cast<uint32_t>(poolSizes.size());
    poolInfo.pPoolSizes = poolSizes.data();
    poolInfo.maxSets = static_cast<uint32_t>(GraphicsContextDesktop::MAX_FRAMES_IN_FLIGHT); // This should be based on the number of expected materials
...
void GraphicsContextDesktop::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
void GraphicsContextDesktop::createImage(uint32_t width,
                                  uint32_t height,
                                  uint32_t mipLevels, 
...
VkImageView GraphicsContextDesktop::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
VkCommandBuffer GraphicsContextDesktop::beginSingleTimeCommands() {
...
void GraphicsContextDesktop::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void GraphicsContextDesktop::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
VkShaderModule GraphicsContextDesktop::createShaderModule(const utils::FileData& file) {
    VkShaderModuleCreateInfo createInfo{};
    createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    createInfo.codeSize = file.size;
    createInfo.pCode = reinterpret_cast<const uint32_t*>(file.data.get());
...
void GraphicsContextDesktop::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void GraphicsContextDesktop::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
bool GraphicsContextDesktop::hasStencilComponent(VkFormat format) {
...
VkSampler GraphicsContextDesktop::createSampler() {
    VkSampler sampler;
    ...
    samplerInfo.maxLod = 0.0f;// static_cast<float>(1); // static_cast<float>(mMipLevels_);
    samplerInfo.mipLodBias = 0.0f;

    if (vkCreateSampler(mDevice_, &samplerInfo, nullptr, &sampler) != VK_SUCCESS) {
        throw std::runtime_error("failed to create texture sampler!");
    }
    return sampler;
}

void GraphicsContextDesktop::populateImage(VkImage image, utils::ImageData& imageData) {
    ...
    transitionImageLayout(
        image,
        VK_FORMAT_R8G8B8A8_SRGB, // TODO this should be a parameter
...
VkShaderModule GraphicsContextDesktop::createShader(const ShaderCreateInfo& shaderCI) {
    VkShaderModule shaderModule{};
    ...
    if (vkCreateShaderModule(mDevice_, &shaderModuleCI, nullptr, &shaderModule) != VK_SUCCESS) {
        throw std::runtime_error("failed to create shader module!");
    }
