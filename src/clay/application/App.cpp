...
void App::mainLoop() {
    while (mWindow_.isRunning()) {
        ...
        mScene_.update(dt.count());
        //const InputHandler& inputHandler = *mWindow_.getInputHandler();
        drawFrame();
    }
...
void App::recreateSwapChain() {
    auto [width, height] = mWindow_.getDimensions();

    while (width == 0 || height == 0) {
        std::tie(width, height) = mWindow_.getDimensions();
        glfwWaitEvents();
    }
...
void App::createSurface() {
    mSurface_ = mWindow_.createSurface(mInstance_);
}
...
void App::createGraphicsPipeline() {
    auto vertShaderCode = readFile(std::string(RESOURCE_PATH) + "/shaders/27_shader_depth.vert.spv");
    auto fragShaderCode = readFile(std::string(RESOURCE_PATH) + "/shaders/27_shader_depth.frag.spv");
    ...
    multisampling.rasterizationSamples = mMSAASamples_;
    ...
    if (vkCreateGraphicsPipelines(mDevice_, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &mMaterial_.mGraphicsPipeline_) != VK_SUCCESS) {
...
void App::createTextureImage() {
    ...
    mMipLevels_ = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;
    ...
    createImage(
        ...
        texHeight, 
        mMipLevels_, 
        VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 
        mTextureImage_, 
        mTextureImageMemory_
    );

    transitionImageLayout(mTextureImage_, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mMipLevels_);
    ...
    generateMipmaps(mTextureImage_, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mMipLevels_);
}
...
void App::createTextureImageView() {
    mTextureImageView_ = createImageView(mTextureImage_, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mMipLevels_);
}
...
void App::loadModel() {
    mScene_.initialize();
    mVertices_.insert(mVertices_.end(), mScene_.mModels_[0]->mVertices_.begin(), mScene_.mModels_[0]->mVertices_.end());
    mIndices_.insert(mIndices_.end(), mScene_.mModels_[0]->mIndices_.begin(), mScene_.mModels_[0]->mIndices_.end());
}
...
void App::createUniformBuffers() {
    ...
    mUniformBuffers_.resize(MAX_FRAMES_IN_FLIGHT);
    mUniformBuffersMemory_.resize(MAX_FRAMES_IN_FLIGHT);
    mUniformBuffersMapped_.resize(MAX_FRAMES_IN_FLIGHT);
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        createBuffer(
            ...
            mUniformBuffers_[i], 
            mUniformBuffersMemory_[i]
        );
        
        vkMapMemory(mDevice_, mUniformBuffersMemory_[i], 0, bufferSize, 0, &mUniformBuffersMapped_[i]);
    }
}
...
void App::createDescriptorSets() {
    std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, mDescriptorSetLayout_);
    ...
    mMaterial_.mDescriptorSets_.resize(MAX_FRAMES_IN_FLIGHT);
    if (vkAllocateDescriptorSets(mDevice_, &allocInfo, mMaterial_.mDescriptorSets_.data()) != VK_SUCCESS) {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        descriptorWrites[0].dstSet = mMaterial_.mDescriptorSets_[i];
        ...
        descriptorWrites[1].dstSet = mMaterial_.mDescriptorSets_[i];
...
void App::createCommandBuffers() {
    mCommandBuffers_.resize(MAX_FRAMES_IN_FLIGHT);
...
void App::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    renderPassInfo.framebuffer = mSwapChainFramebuffers_[imageIndex];
    ...
    vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mMaterial_.mGraphicsPipeline_);
        ...
        vkCmdBindIndexBuffer(commandBuffer, mIndexBuffer_, 0, VK_INDEX_TYPE_UINT32);
        ...
        vkCmdBindDescriptorSets(
            ...
            0, 
            1, 
            &mMaterial_.mDescriptorSets_[mCurrentFrame_], 
            0, 
            nullptr
        );
...
void App::updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.view = mScene_.mCamera_.getViewMatrix();
    ubo.proj = mScene_.mCamera_.getProjectionMatrix();

    memcpy(mUniformBuffersMapped_[currentImage], &ubo, sizeof(ubo));
}
...
void App::drawFrame() {
    ...
    VkResult result = vkAcquireNextImageKHR(
        ...
        mSwapChain_, 
        UINT64_MAX,
        mImageAvailableSemaphores_[mCurrentFrame_], 
        VK_NULL_HANDLE, 
        &imageIndex
    );
    ...
    updateUniformBuffer(mCurrentFrame_);

    vkResetFences(mDevice_, 1, &mInFlightFences_[mCurrentFrame_]);

    vkResetCommandBuffer(mCommandBuffers_[mCurrentFrame_], /*VkCommandBufferResetFlagBits*/ 0);
    recordCommandBuffer(mCommandBuffers_[mCurrentFrame_], imageIndex);
    ...
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &mCommandBuffers_[mCurrentFrame_];
...
void App::cleanup() {
    ...
    vkDestroyPipeline(mDevice_, mMaterial_.mGraphicsPipeline_, nullptr);
    ...
    vkDestroyBuffer(mDevice_, vertexBuffer, nullptr);
